...
static int createTriStrip(int rows, int cols, float size, GLuint &vao)
{
  ...
  std::uniform_int_distribution<int> distribution(0, maxRand);
  ...
  std::vector<glm::vec3> faces;         //вектор граней (треугольников), каждая грань - три индекса вершин, её составляющих; используется для удобства расчета нормалей
  ...
  for (int z = 0; z < rows; ++z)
  {
    for (int x = 0; x < cols; ++x)
    {
      ...
      float yy = float(distribution(generator) - maxRand / 2) / maxRand;
      //std::cout << yy << ' ' << distribution(generator) << std::endl;
  ...
  for (int i = 0; i < faces.size(); ++i)
  {
    ...
    glm::vec3 edge1A(normalize(B - A));
    glm::vec3 edge2A(normalize(C - A));

    glm::vec3 edge1B(normalize(A - B));
    glm::vec3 edge2B(normalize(C - B));

    glm::vec3 edge1C(normalize(A - C));
    glm::vec3 edge2C(normalize(B - C));

    //нормаль к треугольнику - векторное произведение любой пары векторов из одной вершины
    glm::vec3 face_normal = cross(edge1A, edge2A);
  ...
  {
    glm::vec3 N = normalize(normals_vec_tmp.at(i));
...
int main(int argc, char** argv)
{
    ...
    GLFWwindow*  window = glfwCreateWindow(WIDTH, HEIGHT, "OpenGL area generation", nullptr, nullptr);
	if (window == nullptr)
	{
  ...
  glEnable(GL_DEPTH_TEST);  GL_CHECK_ERRORS;
	...
	glm::mat4 model; //начинаем с единичной матрицы
